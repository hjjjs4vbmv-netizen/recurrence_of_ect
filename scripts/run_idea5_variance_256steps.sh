...
RUNS_ROOT="${ECT_RUNS_ROOT:-${PROJECT_ROOT}/runs/idea5-variance-256steps}"
...
[[ -z "$(git status --porcelain)" ]] || {
    echo "Refusing formal training from a dirty worktree." >&2
    exit 1
}

run_in_env python -c "import matplotlib"

GIT_SHA="$(git rev-parse HEAD)"
RUN_DIR="${RUNS_ROOT}/seed${SEED}-${GIT_SHA:0:8}-$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "${RUN_DIR}"
printf '%s\n' "${RUN_DIR}" > "${RUNS_ROOT}/latest_run_dir.txt"

echo "Idea 5 256-step run directory: ${RUN_DIR}"
...
run_in_env python - "${RUN_DIR}" "${MAX_STEPS}" <<'PY'
import csv
import json
import sys
...
if len(rows) != expected:
    raise SystemExit(f"expected {expected} attempted iterations, found {len(rows)}")
last = rows[-1]
if int(last["attempted_iteration"]) != expected:
...
print(f"PASS: exact {expected}-attempt Idea 5 run")
...
run_in_env python     "${ROOT_DIR}/scripts/monitor_idea5_variance_run.py"     "${RUN_DIR}"     --output "${RUN_DIR}/live_training.png"     --once

run_in_env python     "${ROOT_DIR}/scripts/summarize_idea5_variance_run.py"     "${RUN_DIR}"     --output "${RUN_DIR}/visualization"

echo "PASS: Idea 5 formal 256-step training and visualization completed"
echo "Results: ${RUN_DIR}"
echo "Dashboard: ${RUN_DIR}/live_training.png"
echo "Visualization bundle: ${RUN_DIR}/visualization"
