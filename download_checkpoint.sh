...
OUTPUT="${ECT_TRANSFER_PATH:-${PROJECT_ROOT}/pretrained/edm-cifar10-32x32-uncond-vp.pkl}"
REPORT_PATH="${ECT_CHECKPOINT_REPORT:-${PROJECT_ROOT}/runs/day2/checkpoint.json}"
...
Usage: bash download_checkpoint.sh [options]
  ...
  --report PATH       Validation report (default: /mnt/ect_project/runs/day2/checkpoint.json)
...
mkdir -p "$(dirname "${OUTPUT}")" "$(dirname "${REPORT_PATH}")"
if [[ -e "${OUTPUT}" ]]; then
