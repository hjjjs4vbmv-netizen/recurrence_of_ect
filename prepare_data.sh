...
OUTPUT="${ECT_DATA_PATH:-${PROJECT_ROOT}/datasets/cifar10-32x32.zip}"
REPORT_PATH="${ECT_DATA_REPORT:-${PROJECT_ROOT}/runs/day2/dataset.json}"
...
Usage: bash prepare_data.sh [options]
  ...
  --report PATH       Validation report (default: /mnt/ect_project/runs/day2/dataset.json)
...
mkdir -p "$(dirname "${TARBALL}")" "$(dirname "${OUTPUT}")" "$(dirname "${REPORT_PATH}")"
